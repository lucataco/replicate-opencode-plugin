...
# Download plugin
curl -sSL "$REPO_RAW/.opencode/plugins/replicate.ts" -o .opencode/plugins/replicate.ts

# Download skill
curl -sSL "$REPO_RAW/.opencode/skills/replicate/SKILL.md" -o .opencode/skills/replicate/SKILL.md

# Handle package.json -- don't clobber existing
if [ ! -f .opencode/package.json ]; then
  curl -sSL "$REPO_RAW/.opencode/package.json" -o .opencode/package.json
  ...
  echo "  Add this to your dependencies: \"@opencode-ai/plugin\": \"1.3.3\""
fi
